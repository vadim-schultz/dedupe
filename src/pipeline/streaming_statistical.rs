...
impl StreamingStage for StreamingStatisticalStage {
    async fn process_stream(
            ...
            match message {
                StreamingMessage::File(file) => {
                    if let Some(processed_file) = self.process_file(file).await? {
                        output
                            .send(StreamingMessage::File(processed_file))
                            .await
                ...
                StreamingMessage::Batch(files) => {
                    ...
                    if !processed_files.is_empty() {
                        output
                            .send(StreamingMessage::Batch(processed_files))
                            .await
                            .map_err(|_| PipelineError::ChannelSend)?;
                    }
                }
        ...
        debug!(
            "StreamingStatisticalStage completed, processed {} files",
            self.processed_count
        );
        Ok(())
    }
        ...
        match tokio::fs::File::open(&file.path).await {
            Ok(mut fs_file) => {
                ...
                match fs_file.read(&mut buffer).await {
                    ...
                    Err(e) => {
                        warn!(
                            "Failed to read file for statistical analysis {}: {}",
                            file.path, e
                        );
                        Ok(Some(file))
                    }
            ...
            Err(e) => {
                warn!(
                    "Failed to open file for statistical analysis {}: {}",
                    file.path, e
                );
                Ok(Some(file))
            }
