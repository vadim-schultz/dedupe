use crate::{
    config::Config,
    pipeline::streaming::{ProgressTrackingStage, StreamingMessage, StreamingStage},
...
impl StreamingStage for StreamingHashStage {
    async fn process_stream(
        ...
        while let Some(message) = input.recv().await {
            match message {
                ...
                StreamingMessage::Flush => {
                    ...
                    if !groups.is_empty() {
                        output
                            .send(StreamingMessage::Groups(groups))
                            .await
                            .map_err(|_| PipelineError::ChannelSend)?;
                    }
                    output
                        .send(StreamingMessage::Flush)
                        .await
                        .map_err(|_| PipelineError::ChannelSend)?;
                }
                StreamingMessage::Shutdown => {
                    ...
                    output
                        .send(StreamingMessage::Shutdown)
                        .await
                        .map_err(|_| PipelineError::ChannelSend)?;
                    break;
                }
            ...
            if let Some(pb) = &progress_bar {
                pb.set_message(ProgressTracker::stage_progress(
                    "Hash",
        ...
        if let Some(pb) = progress_bar {
            pb.finish_with_message(ProgressTracker::stage_complete(
                "Hash",
                self.processed_count,
            ));
        }
        ...
        debug!(
            "StreamingHashStage completed, processed {} files",
            self.processed_count
        );
        Ok(())
    }
...
impl StreamingHashStage {
    ...
    async fn finalize_groups(&mut self) -> Result<Vec<DuplicateGroup>, PipelineError> {
        ...
        info!(
            "Finalizing groups from {} size groups",
        ...
        for (_size, files) in size_groups {
            ...
            for file in files {
                ...
                quick_groups.entry(key).or_insert_with(Vec::new).push(file);
            }
            ...
            for (_, qc_files) in quick_groups {
                ...
                for file in qc_files {
                    ...
                    stat_groups.entry(key).or_insert_with(Vec::new).push(file);
                }
    ...
    async fn group_by_full_hash(
        &self,
        files: Vec<FileInfo>,
    ) -> Result<Vec<Vec<FileInfo>>, PipelineError> {
    ...
    fn calculate_group_similarity(&self, files: &[FileInfo]) -> f64 {
        ...
        for i in 0..files.len() {
            for j in i + 1..files.len() {
                ...
                if let (Some(st1), Some(st2)) =
                    (&files[i].statistical_info, &files[j].statistical_info)
                {
